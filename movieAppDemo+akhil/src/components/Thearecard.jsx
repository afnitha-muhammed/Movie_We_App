...
import { deleteTheatreApi, getTheatreApi, getMovieApi, addScheduleApi, getScheduleApi } from '../services/allApi';
          ...
          <Card.Body>
            <div className="d-flex justify-content-between">
              ...
              <div  className='ms-auto'>
                <button value={item.id} type="button" className="btn btn-dark" fdprocessedid="szfewf" onClick={()=>{handleshowmodal(item.id)}}>Schedule</button>
                <Button variant="secondary" className='ms-2' onClick={() => viewDetails(item.id) }>View Detaile
        ...
        scheduledetaile?.map((itemm)=>(
                  <div className="col-md-4 col-sm-12">
                    <div className='d-flex align-items-left mt-3'>
                      <div className='p-2 border border-primary'>
                          <h5 className='text-primary'>{itemm.Show_Time}</h5>
                          <h5 className='text-primary'>{itemm.Movie_Name}</h5>
                      </div>
         ...
         <p>No Schedule Yet</p>
            ...
            <form className='p-3'>
              ...
              <select className="form-select" id="exampleSelect1" fdprocessedid="8p7ai8" onChange={(e)=>setSchedule({...Schedule,Movie_Name:e.target.value})} >
                <option>Select...</option>
              {  Movies?.length>0?
                    Movies?.map((itemmovie)=>( <option>{itemmovie.Title}</option>))
              ...
              <label>Show Time</label>
                <input type="time" className='form-control' onChange={(e)=>setSchedule({...Schedule,Show_Time:e.target.value})} />
