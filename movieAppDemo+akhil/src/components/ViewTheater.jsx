...
function ViewTheater({TheatreObject}) {
  ...
  const getschedule = async()=>
      ...
      const selSchedule = allSchedule.filter(item=>item.Movie_Name==movieName)
          ...
          <div className="card my-3">
            <div className="card-body">
              ...
              t.map((teatre)=>(
                ...
                {schedule?.filter((ele)=>ele.theatre_id==teatre.id).map((item)=>(
                  <Link to={'/seatselection'}><button type="button" class="btn btn-outline-danger ms-2">{item.Show_Time}</button> </Link> 
                )
                )}
